...
describe('calculateQuote', () => {
    ...
    test('NEGATIVE Value, possitive Risk', () => {
        //Arrange
        const result = (0, carInsuranceService_1.calculateQuote)(-1001, 5);
        //Act-Assert
        expect(result).toEqual({ error: 'Invalid input. The values cannot be negative or zero' });
    });
    test('Positive value NEGATIVE Risk', () => {
        //Arrange
        const result = (0, carInsuranceService_1.calculateQuote)(3301, -3);
        //Act-Assert
        expect(result).toEqual({ error: 'Invalid input. The values cannot be negative or zero' });
    });
    test('Double Negative', () => {
        //Arrange
        const result = (0, carInsuranceService_1.calculateQuote)(-15781, -3);
        //Act-Assert
        expect(result).toEqual({ error: 'Invalid input. The values cannot be negative or zero' });
    });
